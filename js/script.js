...
// 9 При сворачивании повернуть стрелку налево +++++
...
const emptyInput = () => {
	if ($('#title').val() == "" || $('#discription').val() == "") {
		if ($('#title').val() == "") { alert('Заполните поле Название дела')};
		if ($('#discription').val() == "") {alert('Заполните поле Описание дела'); };
	}
	else {
		$('.warning').remove();
		confirmButton();
...
$(function () {
	...
	$('.wrapper-todo').on('click', '.collapse, .remove', function () {
		...
		if (whatClass == "remove") {
			...
			// удаление с изменением прозрачности за 1000мс при помощи метода fadeout (плавное изменеие прозрачности)
			$('.' + classDel).fadeOut(1000);
		}
		// в другом случае сворачиваем описание
			...
			if ($('.' + colapse).is(':visible')) {
				// скрываем описание с изменением прозрачности за 1000мс при помощи метода fadeout (плавное изменеие прозрачности)
				$('.' + colapse).fadeOut(1000);
				// поворачивает изображение стрелки влево
				$('.' + classCollapseParent + ' .collapse').css('transform', 'rotate(90deg)');
				}
			else {
				// показываем описание с изменением прозрачности за 1000мс при помощи метода fadeIn (плавное изменеие прозрачности)
				$('.' + colapse).fadeIn(1000);
				// поворачивает изображение стрелки вниз
				$('.' + classCollapseParent + ' .collapse').css('transform', 'rotate(0deg)');
				}
			};
	});
});
